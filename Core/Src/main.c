...
volatile uint8_t led3_state = 1;
volatile uint32_t last_btn_tick = 0;
volatile uint32_t led3_period = 1000;
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
  if (htim->Instance == TIM2) {
    led3_counter += 10;  // Timer2 set each 10ms
    if (led3_counter >= led3_period) {
      led3_counter = 0;
      led3_state ^= 1;  // Toggle LED
...
static void MX_TIM4_Init(void)
{
  ...
  sConfigOC.Pulse = 0;  // start from 0
